...
export function PlansModal({ isOpen, onClose }: PlansModalProps) {
  ...
  const handlePayMongoPayment = async (planId: string, planName: string, amount: number) => {
    if (amount === 0) {
    ...
    try {
      ...
      const checkoutUrl = await createPayMongoCheckout(planId, planName, amount, billingCycle);
      ...
      <DialogContent className="max-w-[95vw] h-[92vh] overflow-hidden p-0 flex flex-col">
        <DialogHeader className="px-6 py-3 border-b border-border bg-gradient-to-r from-blue-50 to-violet-50 dark:from-blue-950/20 dark:to-violet-950/20 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-xl">Choose Your Perfect Plan</DialogTitle>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="inline-flex items-center gap-1.5 p-0.5 bg-muted rounded-md border border-border">
                ...
                >
                  Yearly
                  <Badge className="bg-green-600 text-white text-[9px] px-1 py-0">17% OFF</Badge>
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 px-6 py-4 flex flex-col justify-between overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-4 flex-shrink-0">
            ...
            {(adminPlans.length > 0 ? adminPlans : plans).map((plan: any) => {
              ...
              const planPrice = isAdminPlan ? plan.price : (billingCycle === 'monthly' ? plan.price.monthly : plan.price.yearly);
              const planDescription = plan.description;
              const planFeatures = isAdminPlan 
                ? plan.features.filter((f: any) => f.included).map((f: any) => f.text)
                : plan.features;
              const isPopular = isAdminPlan ? plan.isPopular : plan.popular;
              const savings = !isAdminPlan && billingCycle === 'yearly' ? getSavings(plan) : null;
              const isProcessing = isProcessingPayment && selectedPlan === plan.id;
              ...
              return (
                ...
                >
                  ...
                  <CardHeader className="text-center pb-2 pt-3 flex-shrink-0">
                    ...
                    <CardTitle className="text-base mb-0.5">{planName}</CardTitle>
                  ...
                  <CardContent className="space-y-2 px-3 pb-3 flex-1 flex flex-col">
                    ...
                    <div className="text-center py-2 bg-gradient-to-br from-blue-50 to-violet-50 dark:from-blue-950/20 dark:to-violet-950/20 rounded-md border border-blue-200 dark:border-blue-800 flex-shrink-0">
                      <div className="flex items-baseline justify-center gap-0.5">
                        ...
                        <span className="text-muted-foreground text-[10px]">
                          /{isAdminPlan ? (plan.billingPeriod === 'monthly' ? 'mo' : plan.billingPeriod === 'yearly' ? 'yr' : 'life') : (billingCycle === 'monthly' ? 'mo' : 'yr')}
                        </span>
                      </div>
                      {savings && (
                    ...
                    {/* CTA Button */}
                    <Button
                      onClick={() => handlePayMongoPayment(plan.id, planName, planPrice)}
                    ...
                    >
                      {isProcessing ? (
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {plan.id === 'enterprise' ? 'Upgrade Plan' : `Upgrade Pro`}
                    ...
                    <div className="space-y-1.5 pt-1.5 border-t border-border flex-1">
                      ...
                      <ul className="space-y-1">
                        {planFeatures.slice(0, 6).map((feature: string, index: number) => (
                          <li key={index} className="flex items-start gap-1.5">
                            ...
                            <span className="text-[10px] text-foreground leading-tight line-clamp-1">{feature}</span>
                          </li>
                        ))}
                      ...
                      {/* Limitations */}
                      {!isAdminPlan && plan.limitations && plan.limitations.length > 0 && (
                        <ul className="space-y-0.5 pt-1 border-t border-border/50 mt-1">
                          {plan.limitations.slice(0, 2).map((limitation: string, index: number) => (
                            <li key={index} className="flex items-start gap-1.5">
                              <X className="w-2.5 h-2.5 text-gray-400 flex-shrink-0 mt-0.5" />
                              <span className="text-[9px] text-muted-foreground leading-tight">{limitation}</span>
                            </li>
                          ))}
                        </ul>
                      )}
